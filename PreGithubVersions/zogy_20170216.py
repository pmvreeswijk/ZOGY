...
def optimal_subtraction(new_fits, ref_fits):
    ...
    if (not os.path.isfile(sexcat_new) or redo):
        fwhm_new, fwhm_std_new = run_sextractor(base_new+'.fits', sexcat_new, 'Config/sex.config',
                                                'Config/sex.params', fraction=fwhm_imafrac)
    ...
    if (not os.path.isfile(sexcat_ref) or redo):
        fwhm_ref, fwhm_std_ref = run_sextractor(base_ref+'.fits', sexcat_ref, 'Config/sex.config',
                                                'Config/sex.params', fraction=fwhm_imafrac)
    ...
    if not os.path.isfile(ref_fits_remap) or redo:
        result = run_remap(base_new+'_wcs.fits', base_ref+'_wcs.fits', ref_fits_remap,
                           [ysize_new, xsize_new], gain=gain_new, config='Config/swarp.config')
    ...
    for nsub in range(nsubs):
        ...
        if addfakestar:
            ...
            xpos = xsize_fft/2
            ypos = ysize_fft/2
            data_new[nsub][ypos-psf_size/2:ypos+psf_size/2,
                           xpos-psf_size/2:xpos+psf_size/2] += psf_orig_new[nsub]
...
def prep_optimal_subtraction(input_fits, nsubs, imtype):
    ...
    # image before remapping should be read
...
def get_psf(image, ima_header, nsubs, imtype):
    ...
    psf_size = np.int(psf_size_orig * psf_samp + 0.5)
    psf_ima_orig = np.ndarray((nsubs,psf_size,psf_size), dtype='float32')
...
def run_wcs(image_in, image_out, ra, dec, gain, readnoise, fwhm):
    ...
    cmd_sex = 'sex -SEEING_FWHM '+str(seeing)
...
def run_sextractor(image, cat_out, file_config, file_catparams, fitpsf=False,
    ...
    """Function that runs SExtractor on [image], and saves the output
       ...
       catalogue [file_catparams]. If [fitpsf] is set to True,
    ...
    if fraction < 1.:
        ...
        print 'time reading header and full image', time.time()-t1
        ...
        print 'time determining cutout', time.time()-t1
    ...
    cmd = ['sex', image, '-c', file_config, '-CATALOG_NAME', cat_out, 
           '-PARAMETERS_NAME', file_catparams, '-PIXEL_SCALE', str(pixelscale)]
    #'-SEEING_FWHM', str(3.7)]
...
def get_fwhm (cat_ldac, fraction, class_Sort = False):
    ...
    fwhm_mean, fwhm_median, fwhm_std = sigma_clipped_stats(fwhm_select, sigma=3.)
    if verbose:
